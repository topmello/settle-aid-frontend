...
import { LoginData, RegisterData, loginUser, logoutUser, registerUser, selectUserToken } from "../store/authSlice";
...
export const useSession = () => {
  ...
  const tokenExpiresAt = useSelector((state: any) => state.auth?.tokenExpiresAt);
  const refreshToken = useSelector((state: any) => state.auth?.refreshToken);
  const refreshTokenExpiresAt = useSelector((state: any) => state.auth?.refreshTokenExpiresAt);
  ...
  const authenticated = useMemo(() => {
    ...
    if (Date.now() > new Date(tokenExpiresAt).getTime()) {
      ...
      if (Date.now() > new Date(refreshTokenExpiresAt).getTime()) return false;
